<template>
  <div>  
    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-8">
                <nav class="mt-4" area-label="breadcrumb">
...
export default {
    ...
    async asyncData({params}) {
        console.log(params);
        const post = await axios.get(`http://localhost:8000/api/posts/${params.slug}/`);
        const tags = await axios.get(`http://127.0.0.1:8000/api/tags/`);
        const aside = await axios.get(`http://127.0.0.1:8000/api/aside/`);
        const comments = await axios.get(`http://localhost:8000/api/comments/${params.slug}/`);
        return {
            post: post.data,
            tags: tags.data,
            aside: aside.data,
